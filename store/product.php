...
// For moldings, prefer the piece length as the package coverage to avoid over-inflating
// per-package pricing when legacy coverage_per_box values are very large.
if(!$isFlooring && $lengthFtValue !== null){
  $coveragePerBoxValue = $lengthFtValue;
}
...
<?php include $base.'includes/footer.php'; ?>
  <script>
    ...
    function updateCalc(source){
      ...
      const availableBoxesRaw = Number.isFinite(STOCK_AVAILABLE)
        ? (coveragePerPackage > 0 ? STOCK_AVAILABLE / coveragePerPackage : STOCK_AVAILABLE)
        : null;
      const availableBoxes = Number.isFinite(availableBoxesRaw) ? Math.round(availableBoxesRaw) : null;
